...
export async function verifyRouteGuard(req: Request, res: Response, next: NextFunction) {
  const token = req.query.token || req.query.code; // Check whether token or a code is provided
  ...
  const [pendingUser] = await db
    .select()
    .from(pendingUsers) // Ensure this is the correct model/table
    .where(eq(pendingUsers.verificationToken, token))
    .limit(1);
  ...
  req.pendingUser = pendingUser; // Add the user to the request object
...
export async function checkVerificationToken(req: Request, res: Response) {
  ...
  if (!token) {
    return res.status(400).json({ error: 'Token or sendEmail required' });
  }
